...
public class InputMeterSpawner : MonoBehaviour
{
    ...
    void SpawnObjects()
    {
        ...
        if (int.TryParse(inputField.text, out int numberOfObjects))
        {
            for (int i = 0; i < numberOfObjects; i++)
            {
                ...
                // Position the new object (from top to bottom)
                newObj.transform.localPosition = new Vector3(-1610, 680 - i * 100, 0);

                // Assign a unique ID and display it
                InputID inputID = newObj.GetComponent<InputID>();
                inputID.id = currentID++;
                inputID.UpdateIDText();

                // Optionally, set the name of the new object to include its ID
                newObj.name = "Object_" + inputID.id;
